...
const emptyContext = { constants: {}, fetchables: {}, urls: {} };
...
describe("UserTaskExecutor", () => {
  it("returns IN_PROGRESS to signal the node is awaiting user input", async () => {
    ...
    const result = await executor.execute(node, emptyContext, tx);
  ...
  it("evaluates requestMap FEEL expressions and stores results in requestData", async () => {
    const ctx = {
      constants: { orderId: "ord-123", amount: 999 },
      fetchables: {},
      urls: {},
    };
    const node = makeNode({
  ...
  it("throws DataIntegrityError when node configuration is invalid", async () => {
    ...
    await expect(executor.execute(node, emptyContext, tx)).rejects.toThrow(
      DataIntegrityError,
    );
  });
});
